...
{EMOJI['idea']} Используйте кнопки ниже для управления финансами:
    ...
    await msg.reply_text(welcome_text, reply_markup=get_main_keyboard())
...
def _build_distribution_content(user_id):
    """Строит контент обзора и рекомендаций. Возвращает (overview_text, rec_text, keyboard) или (None, error_text, None)."""
    user_data = get_user_data(user_id)
...
{EMOJI['balance']} *Остаток:* {format_money(month_income - total_spent)}
    ...
    if mode == 'current':
        if user_data['incomes']:
            ...
            return None, f"{EMOJI['warning']} Нет данных для рекомендаций!", None
    elif mode == 'average':
        ...
        if recent_incomes:
            ...
            return None, f"{EMOJI['warning']} Недостаточно данных за 3 месяца!", None
    else:
    ...
    if income_amount <= 0:
        return None, f"{EMOJI['info']} Добавьте доходы, чтобы увидеть рекомендации.", None

    family_count = settings.get('family_count', 1)
    goal_percent = settings.get('goal_contribution_percent', 0.1)
    display_period_name = {'day': 'сегодня', 'week': 'неделю', 'month': 'месяц'}.get(period, 'месяц')
    ...
    if income_amount >= total_min_needs:
        ...
        month_start_calc = datetime.now().replace(day=1, hour=0, minute=0, second=0)
        expenses = [e for e in user_data['expenses'] if datetime.fromisoformat(e['date']) >= month_start_calc]
...
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
    ...
    return overview_text, rec_text, InlineKeyboardMarkup(keyboard)

async def show_distribution(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показать ОБЗОР и рекомендации"""
    user_id = update.effective_user.id
    overview_text, rec_text, keyboard = _build_distribution_content(user_id)

    if keyboard is None:
        await update.message.reply_text(rec_text, parse_mode='Markdown')
        return

    await update.message.reply_text(overview_text.strip(), parse_mode='Markdown')
    await update.message.reply_text(rec_text, parse_mode='Markdown', reply_markup=keyboard)
...
async def stats_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"{EMOJI['stats']} *Выберите период статистики:*"
...
async def settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, skip_answer: bool = False):
    """Меню настроек"""
    user_id = update.effective_user.id
    user_data = get_user_data(user_id)
...
async def save_rec_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await settings_menu(update, context, skip_answer=True)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'new_goal':
        ...
        overview_text, rec_text, keyboard = _build_distribution_content(update.effective_user.id)
        if keyboard is None:
            await query.edit_message_text(rec_text, parse_mode='Markdown')
        else:
            full_text = overview_text.strip() + "\n" + rec_text
