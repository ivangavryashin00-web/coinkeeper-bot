...
# ТОКЕН BOTFATHER
BOT_TOKEN = '8309201120:AAFcVl6b3kBKB7ywVKbgR5GgX1VomeO2EeI'
...
def get_user_data(user_id):
    ...
    if str(user_id) not in data:
        data[str(user_id)] = {
            ...
            'settings': {
                ...
                'recommendation_mode': 'current',  # current, average, period
                'recommendation_period': 'month'   # day, week, month
            }
        }
        save_data(data)
    ...
    if 'Базовые нужды (Еда, Транспорт)' not in user_data['settings']['categories']:
        user_data['settings']['categories'] = ['Базовые нужды (Еда, Транспорт)', 'Развлечения', 'Одежда', 'Здоровье', 'Образование', 'Другое']
    if 'family_count' not in user_data['settings']:
        user_data['settings']['family_count'] = 1
    if 'goal_contribution_percent' not in user_data['settings']:
        user_data['settings']['goal_contribution_percent'] = 0.10
    if 'recommendation_mode' not in user_data['settings']:
        user_data['settings']['recommendation_mode'] = 'current'
    if 'recommendation_period' not in user_data['settings']:
        user_data['settings']['recommendation_period'] = 'month'
    ...
    # Редактируемые распределения
    if 'distribution_adjustments' not in user_data:
        user_data['distribution_adjustments'] = {
            'savings': 0,
            'buffer': 0,
            'wants': 0
        }
...
def get_main_keyboard():
    """Главная клавиатура: ОБЗОР сверху, остальные в 2 колонки, ИИ ЧАТ внизу"""
    keyboard = [
...
async def add_income_source(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем ID последнего дохода для возможности отмены
    ...
    await update.message.reply_text(
        ...
        f"{EMOJI['info']} Можете отменить эту операцию кнопкой ниже:",
        reply_markup=get_main_keyboard()
    )

    # Показываем инлайн кнопку отмены отдельным сообщением чтобы не ломать основную клаву
    await update.message.reply_text(
        f"Управление операцией:",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

    # Показываем рекомендации по распределению
...
async def add_expense_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверяем, не превысили ли мы рекомендованный бюджет
...
def check_budget_recommendation(user_data, category, amount):
    ...
    # Рекомендуемые проценты по категориям
    recommended = {
        'Еда': 0.30,
        'Транспорт': 0.10,
        'Развлечения': 0.10,
...
async def goals_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"{EMOJI['edit']} Редактировать цель", callback_data='edit_goal')],
...
async def manual_add_to_goal_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Добавляем в историю как перевод на цель
        transfer = {
            'id': len(user_data['expenses']) + 1,
            'amount': amount,
...
async def show_distribution(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала показываем ОБЩИЙ ОБЗОР (баланс и т.д.)
    month_start = datetime.now() - timedelta(days=30)
    month_income = sum(inc['amount'] for inc in user_data['incomes'] 
                      if datetime.fromisoformat(inc['date']) > month_start)
...
{EMOJI['balance']} *Остаток:* {format_money(month_income - total_spent)}
    ...
    # Теперь отдельным, крупным блоком - РЕКОМЕНДАЦИИ
    income_amount = 0
    title_period = ""
    ...
    if mode == 'average' or mode == 'current':
        if period == 'day': income_amount /= 30
        elif period == 'week': income_amount /= 4.3

    needs_multiplier = 0.5 + (family_count - 1) * 0.1
    if needs_multiplier > 0.8: needs_multiplier = 0.8

    # Учитываем фиксированные расходы
    fixed_total = sum(user_data.get('fixed_expenses', {}).values())
    
    # Расчет свободного остатка
    balance = user_data['balance']
    
    # Минимум 500 рублей на человека в день
    min_daily_per_person = 500
    min_total_daily = min_daily_per_person * family_count
    
    # Рекомендация на сегодня (период 30 дней для расчета месячного плана)
    days_in_period = {'day': 1, 'week': 7, 'month': 30}.get(period, 30)
    
    # ПРИОРИТЕТ 1: Базовые нужды (500р/чел в день)
    total_min_needs = min_total_daily * days_in_period
    
    if income_amount >= total_min_needs:
        # Если доход покрывает базу, распределяем остаток
        remaining = income_amount - total_min_needs
        
        # ПРИОРИТЕТ 2: Фиксированные расходы
        ...
        # Применяем ручные корректировки если они есть
        ...
        # Остаток на развлечения и буфер
        ...
        # Получаем статистику трат за текущий месяц для детализации
        ...
        # Детализация для Резерва и Развлечений
        ...
        # Дохода не хватает даже на базу
    ...
    if today_budget < min_total_daily:
        warning_text = f"\n⚠️ *КРИТИЧЕСКИ:* Ваш доход не покрывает базовый минимум ({format_money(min_total_daily)}/день). Приоритет отдан выживанию, цели и аренда пока не учитываются!"
    elif fixed_covered < fixed_total:
        warning_text = f"\n⚠️ *Внимание:* Денег хватает на еду, но недостаточно для полной оплаты аренды/фикс. расходов (не хватает {format_money(fixed_total - fixed_covered)})."
...
{EMOJI['sparkle']} ИИ определил приоритеты: сначала еда ({format_money(min_total_daily)} на чел/день), затем фиксированные расходы и цели.
▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
    """
...
async def stats_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"{EMOJI['chart']} Подробный анализ", callback_data='stats_detailed')]
    ]
...
"""
    ...
    prev_incomes = [inc for inc in user_data['incomes'] 
                    if prev_start <= datetime.fromisoformat(inc['date']) < start_date]
    prev_expenses = [exp for exp in user_data['expenses'] 
                     if prev_start <= datetime.fromisoformat(exp['date']) < start_date]

    prev_income = sum(inc['amount'] for inc in prev_incomes)
    prev_expense = sum(exp['amount'] for exp in prev_expenses)

    if prev_income > 0 or prev_expense > 0:
        text += f"\n\n{EMOJI['refresh']} *Сравнение с прошлым периодом:*"
        income_diff = total_income - prev_income
        expense_diff = total_expense - prev_expense

        inc_emoji = EMOJI['trend_up'] if income_diff >= 0 else EMOJI['trend_down']
        exp_emoji = EMOJI['trend_down'] if expense_diff <= 0 else EMOJI['trend_up']

        text += f"\nДоходы: {inc_emoji} {'+' if income_diff >= 0 else ''}{format_money(income_diff)}"
        text += f"\nРасходы: {exp_emoji} {'+' if expense_diff >= 0 else ''}{format_money(expense_diff)}"
...
async def history_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(f"{EMOJI['edit']} Редактировать", callback_data='edit_operation')],
...
async def delete_operation_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Показываем последние операции для удаления
    keyboard = []
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'new_goal':
        ...
        await query.edit_message_text("Введите сумму ежемесячной аренды (только число):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{EMOJI['back']} Назад", callback_data="settings")]]))
...
async def ai_chat_handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"""
Ты — профессиональный финансовый консультант в боте CoinKeeper. 
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == f"{EMOJI['plus']} Доход":
        context.user_data['waiting_for_rent'] = False # Сбрасываем флаги
...
async def daily_notification_task(context: ContextTypes.DEFAULT_TYPE):
    """Задача для ежедневной рассылки рекомендаций"""
    data = load_data()
    now = datetime.now()
    current_time = now.strftime("%H:%M")
    
    for user_id, user_data in data.items():
        notif_settings = user_data.get('settings', {}).get('notifications', {})
        if notif_settings.get('enabled') and notif_settings.get('time') == current_time:
            # Генерируем текст рекомендаций (упрощенная версия show_distribution)
            settings = user_data['settings']
            period = settings.get('recommendation_period', 'month')
            
            # Для рассылки берем текущий доход или средний
            income_amount = 0
            if user_data['incomes']:
                income_amount = user_data['incomes'][-1]['amount']
            ...
            if income_amount > 0:
                fixed_total = sum(user_data.get('fixed_expenses', {}).values())
                goal_percent = settings.get('goal_contribution_percent', 0.1)
                available_after_fixed = income_amount - fixed_total
                savings = income_amount * goal_percent
                daily_needs_budget = (available_after_fixed - savings) * 0.7
                days_in_period = {'day': 1, 'week': 7, 'month': 30}.get(period, 30)
                today_budget = daily_needs_budget / days_in_period
                
                text = f"{EMOJI['sparkle']} *Ваш план на сегодня:* {EMOJI['sparkle']}\n\n"
                text += f"💳 Сегодня можно потратить: *{format_money(today_budget)}*\n"
                text += f"🏠 Баланс: {format_money(user_data['balance'])}\n\n"
                text += f"{EMOJI['idea']} Старайтесь не выходить за рамки бюджета!"
...
async def daily_notification_callback(context: ContextTypes.DEFAULT_TYPE):
    """Рассылка ежедневных рекомендаций"""
    data = load_data()
    for user_id, user_data in data.items():
        settings = user_data.get('settings', {})
        notif_settings = settings.get('notifications', {})
        
        if notif_settings.get('enabled', True):
            # Формируем текст рекомендации (упрощенная версия show_distribution)
            try:
                # Временно подменяем update для вызова существующей логики или пишем упрощенный расчет
                # Для простоты отправим сообщение напрямую
                income_amount = 0
                if user_data['incomes']:
                    income_amount = user_data['incomes'][-1]['amount']
                
                if income_amount > 0:
                    # Здесь логика расчета аналогична show_distribution
                    family_count = settings.get('family_count', 1)
                    goal_percent = settings.get('goal_contribution_percent', 0.1)
                    fixed_total = sum(user_data.get('fixed_expenses', {}).values())
                    ...
                    savings = income_amount * goal_percent
                    daily_needs_budget = (available_after_fixed - savings) * 0.7
                    if daily_needs_budget < 0: daily_needs_budget = 0
                    
                    today_budget = daily_needs_budget / 30 # Предполагаем месяц для уведомления
                    
                    text = f"{EMOJI['idea']} *Ежедневное напоминание*\n\n"
                    text += f"Сегодня вы можете потратить {format_money(today_budget)} на еду и транспорт.\n"
                    text += f"Ваш текущий баланс: {format_money(user_data['balance'])}"
...
def main():
    ...
    # Настройка JobQueue для уведомлений
    job_queue = application.job_queue
    if job_queue:
        # Проверяем уведомления каждую минуту и отправляем если время совпало
        async def schedule_notifications(context: ContextTypes.DEFAULT_TYPE):
            now = datetime.now().strftime("%H:%M")
            data = load_data()
            for user_id, user_data in data.items():
                user_time = user_data.get('settings', {}).get('notifications', {}).get('time', '09:00')
                if now == user_time and user_data.get('settings', {}).get('notifications', {}).get('enabled', True):
                    await daily_notification_task(context)

        job_queue.run_repeating(schedule_notifications, interval=60, first=10)
    else:
        logger.warning("JobQueue не запущен. Уведомления не будут работать.")
